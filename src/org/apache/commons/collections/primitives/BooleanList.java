...
public interface BooleanList extends BooleanCollection {
  ...
  * than that it already contains the element, it <i>must</i>
  * throw an exception (rather than simply returning {@code false}). This
  ...
  * returns <code>true</code> iff <i>that</i> is a <code>BooleanList</code> that
  * has the same {@link #size() size} as me, and for which the elements returned
  * by its {@link BooleanList#iterator iterator} are equal (<code>==</code>) to
  * the corresponding elements within me. (This contract ensures that this
  * method works properly across different implementations of the
  * <code>BooleanList</code> interface.)
  *
  ...
  * <p >
  * The hash code of an <code>BooleanList</code> is defined to be the result of
  * the following calculation: 	 <pre> int hash = 1;
  ...
  * returned by an initial call to the {@link
  * BooleanListIterator#next next} method. An initial call to the {@link
  * BooleanListIterator#previous previous} method would return the element with
  * the specified <i>index</i> minus one.
  *
  * @param index
  * @return
  * @throws IndexOutOfBoundsException if the specified index is out of range
